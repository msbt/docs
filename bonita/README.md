...
-	Launch the new container pointing towards the copy of DB and filesystem
	...
	```console
	$ docker run --name=bonita_postgres --link mydbpostgres:postgres -e "DB_NAME=newbonitadb" -e "DB_USER=newbonitauser" -e "DB_PASS=newbonitapass" -v "$PWD"/bonita_migration:/opt/bonita/ -d -p 8081:8080 arm32v7/bonita:7.2.4
	```

	-	If >= 7.3.0

	```console
	$ docker run --name=bonita_postgres --link mydbpostgres:postgres -e "DB_NAME=newbonitadb" -e "DB_USER=newbonitauser" -e "DB_PASS=newbonitapass" -d -p 8081:8080 arm32v7/bonita:2021.2-u0
	```

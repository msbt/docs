...
$ docker run -v /path/to/default.vcl:/etc/varnish/default.vcl:ro --tmpfs /var/lib/varnish/varnishd:exec s390x/varnish
```
...
```console
$ docker run --tmpfs /var/lib/varnish/varnishd:exec -e VARNISH_SIZE=2G s390x/varnish
```
...
$ docker run --tmpfs /var/lib/varnish/varnishd:exec -e VARNISH_SIZE=2G s390x/varnish -p default_keep=300
```
